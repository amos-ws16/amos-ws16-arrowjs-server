...
/**
 * Schema of a Token in the database.
 * A token automatically expires after  3600s / 60 min
 */
 ...
 * 3600s / 60min and will automatically be deleted from the DB. The callback
...
function addRequest (nRequest, nResponse, callback) {
  var newRequest = new model.Request({ request: nRequest, response: nResponse })
  newRequest.save((err) => {
    if (err) {
      console.log(err)
      callback(err)
...
/**
 * This method searches in the database to find the request with the given id.
 * The result will be given in the callback method with two arguments:
 * - err: error if it occurs during DB access
 * - doc: the javascript of the request which was found
 */
function findRequest (fID, callback) {
  // Change the given ID to a ObjectID, used by mongo
  var objectID = mongoose.Types.ObjectId(fID)
  model.Request.find({ _id: objectID }, (err, doc) => {
    // return false if item was not found, else return the item
    if (err) {
      callback(false)
      return
    }
