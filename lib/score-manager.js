...
class ScoreManager {
  ...
  scoreWith (pluginId, blob) {
    let args = this.plugins[pluginId].inputs.map(i => extractObject(blob, i))
    this.plugins[pluginId].score.apply(null, args)
  }
   ...
   * to be scored for this file. The plugin should return a number between 0.0
   * and 1.0 where 0.0 means no correlation and 1.0 means total match for the
